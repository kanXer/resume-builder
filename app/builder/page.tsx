...
// Configure the worker (essential for it to run)
...
function ResumeBuilderContent() {
  ...
  useEffect(() => {
    async function fetchTemplateDetails() {
      try {
        ...
        const currentTemplate = data.find((t: any) => t.id === templateId);
        if (currentTemplate) {
          setTemplateName(currentTemplate.name);
        } else {
          setTemplateName(templateId.toUpperCase());
        }
      } catch (err) {
        setTemplateName(templateId.toUpperCase());
      }
  ...
  async function generatePDF() {
    setLoading(true);
    let payload = { ...formData, template: `${templateId}.html` };
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-6 md:py-10 px-4 md:px-8 font-sans pb-32">
      <div className="max-w-6xl mx-auto">
          ...
          <div className="lg:col-span-8 space-y-6 md:space-y-10">
            ...
            <section className={SectionCard}>
              <div className="flex justify-between items-center mb-6 md:mb-8">
                ...
                <button onClick={addExperience} className="bg-orange-50 text-orange-600 px-3 py-1.5 rounded-xl font-black text-[9px] md:text-[10px] uppercase tracking-widest hover:bg-orange-600 hover:text-white transition-all">+ Add Job</button>
              </div>
              {formData.experience.map((exp, i) => (
                <div key={i} className="mb-6 p-4 md:p-6 border border-gray-100 rounded-2xl bg-gray-50/50 relative">
                  ...
                  <div className="space-y-2">
                    ...
                    <button onClick={() => addPoint(i)} className="text-blue-700 font-black text-[9px] uppercase tracking-widest ml-1">+ Add Point</button>
                  </div>
                </div>
              ))}
            </section>
            ...
            <section className={SectionCard}>
              <div className="flex justify-between items-center mb-6 md:mb-8">
                ...
                <button onClick={addEducation} className="bg-purple-50 text-purple-700 px-3 py-1.5 rounded-xl font-black text-[9px] md:text-[10px] uppercase tracking-widest hover:bg-purple-700 hover:text-white transition-all">+ Add Edu</button>
              </div>
              {formData.education.map((edu, i) => (
                <div key={i} className="grid grid-cols-1 md:grid-cols-5 gap-3 mb-4 p-4 border border-gray-100 rounded-2xl bg-gray-50/50 relative">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button onClick={() => removeEducation(i)} className="text-red-400 hover:text-red-600 transition-colors shrink-0"><Trash2 size={18}/></button>
                  </div>
                </div>
              ))}
            </section>
              ...
              <div className="space-y-2">
                {formData.skills.map((s, i) => (
                  <div key={i} className="flex gap-2">
                    ...
                    <button onClick={() => removeArrayItem("skills", i)} className="text-gray-300 hover:text-red-500"><Trash2 size={18}/></button>
                  </div>
                ))}
              </div>
              <button onClick={() => addArrayItem("skills")} className="w-full mt-3 py-2 border-2 border-dashed border-gray-200 rounded-xl text-[9px] md:text-[10px] font-black uppercase tracking-widest text-gray-400 hover:bg-gray-50 transition-all">+ Add Skill</button>
            </div>
            ...
            <div className="bg-white p-5 md:p-6 rounded-2xl shadow-lg border border-gray-100">
              ...
              {formData.strengths.map((s, i) => (
                <div key={i} className="flex gap-2 mb-2">
                  ...
                  <button onClick={() => removeArrayItem("strengths", i)} className="text-gray-300"><Trash2 size={18}/></button>
                </div>
              ))}
              <button onClick={() => addArrayItem("strengths")} className="w-full mt-3 py-2 border-2 border-dashed border-gray-200 rounded-xl text-[9px] md:text-[10px] font-black uppercase tracking-widest text-gray-400">+ Add Strength</button>
            </div>
        ...
        {/* FLOATING ACTION - RESPONSIVE SIZE */}
        <div className="fixed bottom-6 right-6 md:bottom-10 md:right-10 z-[100]">
          <button onClick={generatePDF} disabled={loading} className="bg-blue-700 hover:bg-black text-white px-6 md:px-10 py-4 md:py-5 rounded-2xl md:rounded-[2rem] font-black text-sm md:text-lg flex items-center gap-3 shadow-2xl transition-all active:scale-90 hover:-translate-y-2 uppercase italic">
        ...
        {loading && (
          <div className="fixed inset-0 z-[200] bg-black/60 backdrop-blur-md flex items-center justify-center p-4">
            <div className="bg-white p-8 md:p-12 rounded-[2rem] md:rounded-[3rem] text-center border-b-[10px] border-blue-700 shadow-2xl w-full max-w-xs">
              <div className="w-16 h-16 md:w-20 md:h-20 border-[6px] md:border-[8px] border-blue-700 border-t-transparent rounded-full animate-spin mx-auto mb-6"></div>
              <h2 className="text-lg md:text-xl font-black text-blue-700 italic uppercase">Processing...</h2>
            </div>
          </div>
        )}
        ...
        {pdfUrl && !loading && (
          <div className="fixed inset-0 z-[200] bg-black/95 flex items-center justify-center p-0 md:p-6 backdrop-blur-md">
            <div className="bg-white w-full h-full md:h-[94vh] md:max-w-5xl md:rounded-[3rem] flex flex-col overflow-hidden shadow-2xl border border-white/10 text-black">
              ...
              <div className="p-4 md:p-6 bg-gray-50 flex flex-col md:flex-row justify-between items-center gap-4 border-b border-gray-200">
                <div className="text-center md:text-left hidden md:block">
                  <h2 className="font-black text-xl md:text-2xl flex items-center gap-2 italic uppercase tracking-tighter">
                    <CheckCircle className="text-green-600"/> Resume Ready
                  </h2>
                  <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Style: {templateName}</p>
                </div>

                {/* ZOOM & PAGE CONTROLS - STACK ON MOBILE */}
                <div className="flex items-center bg-white border border-gray-200 px-3 py-1.5 rounded-xl md:rounded-2xl gap-2 md:gap-4 shadow-sm scale-90 md:scale-100">
                  <div className="flex items-center gap-1 md:gap-2">
                    <button onClick={() => setScale(s => Math.max(s - 0.1, 0.4))} className="p-1 hover:bg-gray-100 rounded-lg"><ZoomOut size={16}/></button>
                    <span className="text-[9px] md:text-[10px] font-black w-8 md:w-10 text-center">{Math.round(scale * 100)}%</span>
                    <button onClick={() => setScale(s => Math.min(s + 0.1, 2.0))} className="p-1 hover:bg-gray-100 rounded-lg"><ZoomIn size={16}/></button>
                  </div>
                  <div className="w-px h-4 bg-gray-300" />
                  <div className="flex items-center gap-1 md:gap-2">
                    <button disabled={pageNumber <= 1} onClick={() => setPageNumber(p => p - 1)} className="disabled:opacity-20"><ChevronLeft size={18}/></button>
                    <span className="text-[9px] md:text-[10px] font-black uppercase">Pg {pageNumber} / {numPages}</span>
                    <button disabled={pageNumber >= (numPages || 1)} onClick={() => setPageNumber(p => p + 1)} className="disabled:opacity-20"><ChevronRight size={18}/></button>
                  </div>
                </div>
                
                <div className="flex items-center gap-4 w-full md:w-auto justify-between md:justify-end px-4 md:px-0">
                  <button onClick={() => setPdfUrl(null)} className="font-black text-gray-400 hover:text-red-500 uppercase text-[10px] tracking-widest">Discard</button>
                  <a href={pdfUrl} download={`${formData.name || 'Resume'}_cv_${templateName}.pdf`} className="bg-blue-700 hover:bg-black text-white px-6 py-2.5 rounded-xl font-black text-xs shadow-lg transition-all active:scale-95">DOWNLOAD PDF</a>
                </div>
              </div>

              {/* PDF VIEWER BODY - ADAPTIVE SCROLLING */}
              <div className="flex-1 bg-zinc-800 overflow-auto flex justify-center p-2 md:p-6 scrollbar-hide">
                <div className="shadow-2xl h-fit w-fit">
                  <Document
                    ...
                    loading={
                      <div className="flex flex-col items-center p-10 md:p-20 text-white gap-4">
                        <div className="w-8 h-8 border-4 border-blue-500 border-t-transparent rounded-full animate-spin" />
                        <p className="font-black text-[10px] uppercase tracking-widest italic">Rendering...</p>
                      </div>
                    }
                  >
                    <Page 
...
export default function ResumeBuilder() {
  return (
    <Suspense fallback={<div className="min-h-screen flex items-center justify-center font-black text-xl md:text-2xl bg-gray-50 uppercase italic tracking-tighter">Loading Studio...</div>}>
